...
public class OrderCart extends AppCompatActivity implements ReceiveListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /** GET THE KOT PRINTER IP ADDRESS **/
        db = new DBResto(OrderCart.this);
        String s = "SELECT * FROM " + db.PRINTERS + " WHERE " + db.PRINTER_PRINTS + " = '0'";
        ...
        if (cursor != null && cursor.getCount() != 0)	{
            for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
                ...
                if (cursor.getString(cursor.getColumnIndex(db.PRINTER_IP)) != null)	{
                    PRINTER_IP = cursor.getString(cursor.getColumnIndex(db.PRINTER_IP));
                } else {
                    PRINTER_IP = null;
                }
            }
        ...
        if (bundle.containsKey("TABLE_NO")) {
            ...
            if (INCOMING_TABLE_ID != null)  {
                ...
                String qryOrderCategories =
                        "SELECT * FROM " + db.ORDER_CART + " WHERE " + db.ORDER_TABLE_ID + " = " + INCOMING_TABLE_ID;
                Cursor cursor = db.selectAllData(qryOrderCategories);
//                Log.e("ORDERCART DUMP", DatabaseUtils.dumpCursorToString(cursor));
                if (cursor != null && cursor.getCount() != 0)	{
                        ...
                        if (cursor.getString(cursor.getColumnIndex(db.ORDER_MENU_ID)) != null)	{
                            ...
                            if (curMenu != null && curMenu.getCount() != 0)	{
                                for (curMenu.moveToFirst(); !curMenu.isAfterLast(); curMenu.moveToNext()) {
                                    ...
                                    /** GET THE MEAL IMAGE **/
                                    if (curMenu.getBlob(curMenu.getColumnIndex(db.MENU_IMAGE)) != null)	{
                                        byte[] bytImage = curMenu.getBlob(curMenu.getColumnIndex(db.MENU_IMAGE));
        ...
        try {
            ...
            if (arrOrders.size() != 0)  {
                ...
                for (int i = 0; i < arrOrders.size(); i++) {
                    ...
                    Boolean ORDER_STATUS = arrOrders.get(i).isOrderStatus();
                    if (!ORDER_STATUS)   {
...
//            Log.e("ADAPTER STATUS", String.valueOf(data.isOrderStatus()));
            if (data.isOrderStatus())   {
                holder.txtRemoveDish.setVisibility(View.GONE);
            } else {
                holder.txtRemoveDish.setVisibility(View.VISIBLE);
            }
...
//                    Log.e("STATUS", String.valueOf(data.isOrderStatus()));
    ...
    private boolean connectPrinter() {
        ...
        try {
            mPrinter.connect(PRINTER_IP, Printer.PARAM_DEFAULT);
        }
        catch (Exception e) {
            ...
            public synchronized void run() {
                ...
                if (code == 0)  {
                    ...
                    if (arrOrders.size() != 0) {
                        for (int i = 0; i < arrOrders.size(); i++) {
                            /** GET THE ORDER ID **/
                            String ORDER_ID = arrOrders.get(i).getOrderID();
                            db.updateOrderStatus(ORDER_ID, true);
                        }
                    }
                }
